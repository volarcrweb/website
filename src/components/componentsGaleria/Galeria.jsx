  ...
  <div className="movingtext-container">
    ...
    >
      ...
      >
        ...
        >
          <Stack direction={"row"} spacing={2} flexWrap={"wrap"} justifyContent={"center"} style={{rowGap: isMobile ? "0.5rem" : "0"}}>
          {amenidades.map((amenidad) => (
            <ToggleButton key={amenidad.id} value={amenidad.id} className="toggleButton">
              {amenidad.nombre}
            </ToggleButton>
          ))}
          </Stack>
      ...
      <Stack justifyContent={"center"} alignItems={"flex-end"} style={{ width: "99%", height: "60%", overflow: "visible", position: "relative"}}>
        {/* Carrusel Container */}
        <Stack
          style={{
            position: "relative",
        ...
        <Stack style={{width: isMobile ? "100%" : "50%", height: "100%", }}>
          ...
          {(() => {
            ...
            const amenidadActiva = amenidades.find(amenidad => amenidad.id === alignment);
            
            if (!amenidadActiva) return null;
            
            return (
              <Stack direction={"row"} justifyContent={isMobile ? "center" : "flex-start"} alignItems={"center"} spacing={isMobile ? 3 : 6} style={{paddingTop: isMobile ? "2rem" : "3rem", paddingLeft: isMobile ? "0" : "13em"}}>
                
                
                <Stack direction={"row"} spacing={isMobile ? 3 : 5}>
                  {amenidadActiva.detalle.map((detalle) => (
                    <Stack 
                      key={detalle.id}
                      spacing={1}
                      justifyContent="flex-end"
                    ...
                    >
                      <img src={detalle.imagen} alt={detalle.nombre} style={{width: isMobile ? "2.5rem" : "3.5rem", height: "auto"}}/>
                      
                      <Typography 
                        className="subtituloDetalle"
                      >
                        {detalle.nombre}
                      </Typography>
                    </Stack>
                ...
                <Divider orientation="vertical" flexItem style={{height: "100%", borderColor: "#2D2D2D", borderWidth: isMobile ? "0.05rem" : "0.1575rem"}}/>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {segundaPalabra && (
                          <Typography className="tituloDetalle" style={{paddingLeft: "1rem"}}>
                            {segundaPalabra}
                          </Typography>
                        )}
                      </>
